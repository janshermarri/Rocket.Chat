...
export class DiffSequence {
	...
	static diffQueryChanges<T extends { _id: string }, TProjection extends T = T>(
		...
		newResults: T[],
		observer: Observer<T>,
		options?: { projectionFn?: (doc: Omit<T, '_id'>) => TProjection },
	): void;
	...
	static diffQueryUnorderedChanges<T extends { _id: string }, TProjection extends T = T>(
		...
		options = options || {};
		const projectionFn: (doc: Omit<T, '_id'>) => TProjection = options.projectionFn || EJSON.clone;
		...
		newResults.forEach((newDoc, id) => {
			...
			if (oldDoc) {
				if (observer.changed && !EJSON.equals(oldDoc, newDoc as any)) {
				...
				const fields: Omit<TProjection, '_id'> & { _id?: string } = projectionFn(newDoc);
				delete fields._id;
	...
	static makeChangedFields<TLeft, TRight>(
		...
		DiffSequence.diffObjects<TLeft, TRight>(oldDoc, newDoc, {
			...
			both(key, leftValue, rightValue) {
				if (!EJSON.equals(leftValue as any, rightValue as any)) fields[key] = rightValue;
			},
		});
		return fields;
	}
